#!/usr/bin/env python3
"""Phase 119 — import Collection Nine, Ten, Eleven stories."""
...
SPEC_FILES = [
    REPO / "tools" / "phase119_collection_nine_specs.json",
    REPO / "tools" / "phase119_collection_ten_specs.json",
    REPO / "tools" / "phase119_collection_eleven_specs.json",
]
MANUSCRIPTS = [
    Path(
        "/Users/admin/Developer/MiscellaneousNotes/AwesomeVault/!Apps/App Ideas/alephBits/Books/09 CollectionNine.md"
    ),
    Path(
...
# Collection Eight already imported in Phase 102 — parse only for skip reporting.
COLLECTION_EIGHT = Path(
    "/Users/admin/Developer/MiscellaneousNotes/AwesomeVault/!Apps/App Ideas/alephBits/Books/08 CollectionEight.md"
)
SKIP_MANUSCRIPT_TITLES = {
    "Niedźwiedź\u00a0numer dwa",  # nbsp variant from parser
    "Niedźwiedź numer dwa",
    "Zgaszone oczy",
    "Droga na scenę",
    "Lustrzane Odbicie",
    "Sprawa",  # C11 duplicate — import from C9 only
}
PHASE_DATE = "2026-07-13"
...
def render_motifs(motifs: list[str]) -> str:
    if not motifs:
        return ""
    lines = ["**Motifs:**"]
    lines.extend(f"- {m}" for m in motifs)
    return "\n".join(lines)
...
def render_reading_pack(spec: PackSpec, story: ParsedStory, text_body: str) -> str:
    ...
    src_date = story.source_dates[0] if story.source_dates else PHASE_DATE
    source_url = f"https://www.youtube.com/watch?v={yt}" if yt else "*(none — manuscript)*"
    motifs_block = render_motifs(spec.motifs)
    ...
    lines = [
        ...
        f"**Recommended level:** {max(1, spec.difficulty - 2)}  ",
        "",
        f"**Tags:** {spec.tags}  ",
        "",
    ...
    if motifs_block:
        lines.extend(["", motifs_block, ""])
    lines.extend(
        [
            f"**Editorial notes:** {spec.editorial_notes}",
            "",
            "**Inspiration:** "
            + (spec.inspiration or f"{spec.series} manuscript — editorial adaptation."),
            "",
            "---",
            "",
            ...
            "**LLM assisted:** yes  ",
            "**LLM model:** Claude  ",
            f"**Human reviewed:** yes — {PHASE_DATE}  ",
            ...
            "| Version | Date | Note |",
            "|---------|------|------|",
            f"| 1.0.0 | {PHASE_DATE} | Collection editorial import (Phase 119) |",
            "",
            "### Editorial history",
            "",
            "| Date | Editor | Note |",
            "|------|--------|------|",
            f"| {PHASE_DATE} | AlephBits Editorial | Phase 119 import; philosophy fit {spec.philosophy_stars}/5 — {spec.philosophy_note} |",
            "",
            "---",
            "",
            "## Sources",
...
def write_provenance(pack_dir: Path, spec: PackSpec, story: ParsedStory) -> None:
    ...
    data = {
        ...
        "createdAt": PHASE_DATE,
        "lastReviewedAt": PHASE_DATE,
...
def load_specs() -> dict[str, PackSpec]:
    ...
    for title, data in raw.items():
        ...
        specs[title] = PackSpec(
            ...
            revision_notes=data.get("revision_notes", "Phase 119 import."),
            ...
            series=data.get("series", "Collection Nine"),
        )
    return specs
...
def main() -> None:
    ...
    seen_c9_sprawa = False

    all_manuscripts = MANUSCRIPTS + [COLLECTION_EIGHT]

    for manuscript in all_manuscripts:
        is_c8 = manuscript == COLLECTION_EIGHT
        is_c11 = "CollectionEleven" in manuscript.name
        for story in parse_manuscript(manuscript):
            if is_c8:
                skipped.append(f"{story.title} — Collection Eight already official (Phase 102)")
                continue

            norm_title = story.title.replace("\u00a0", " ")
            if norm_title in {t.replace("\u00a0", " ") for t in SKIP_MANUSCRIPT_TITLES}:
                if story.title == "Sprawa" and is_c11:
                    skipped.append(f"{story.title} (C11) — duplicate of Collection Nine Sprawa")
                elif story.title == "Sprawa" and not is_c11:
                    pass  # import from C9
                else:
                    skipped.append(f"{story.title} — editorial skip (near-duplicate or defer)")
                    continue
            ...
            spec = specs.get(story.title) or specs.get(norm_title)
            if not spec:
    ...
    run_cmd(["dart", "run", "tools/build_manifest.dart", "--overwrite"], REPO)
    ...
    print("Phase 119 import complete.")
